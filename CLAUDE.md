...
Stubea canvas/DOM/window y corre el engine real sin navegador: por cada escena construye `World`, ejecuta `runInit` y luego corre el guion completo hasta el final (`runToCompletionOrHang`, con un click de interacción a mitad), llama `reset()` y tickea de nuevo. Cualquier excepción es un FAIL. Caza los bugs silenciosos de ESM que `node --check` no ve (imports rotos, `this`/`world` mal traducidos, métodos inexistentes). Además **detecta guiones colgados**: si un script se ESTANCA (su `sc.i` no avanza durante ~100 s simulados mientras espera) porque un `waitFor: "arrive"` o un `waitUntil` nunca se cumple (destino inalcanzable, predicado imposible), en vez de "correr sin explotar" ahora es un FAIL con el step atascado. Se mide por estancamiento, no por "no terminó": un bucle sano (`loop: true` o `goto` a un label) sigue moviendo `sc.i` y NO se marca, pero un guion que cuelga ANTES de llegar a su loop sí (por eso el chequeo no exime a los guiones con loop). Correrlo tras cualquier cambio al engine. No reemplaza la verificación visual: no mira píxeles, solo que nada explote ni se congele.
...
Y una capa de **lints anti-fallo-silencioso** (los bugs que "corren" pero se ven mal o cuelgan): (1) **alcanzabilidad** (`checkReachability`): un destino de `walk`/`path` bajo la banda de subtítulos o sobre el horizonte NUNCA se alcanza (el motor clampa al learner ahí). Para un `walk` (walkTo, que llega por proximidad, umbral 4 px) eso significa que un `waitFor: "arrive"` posterior CUELGA el guion para siempre sin error de runtime → ERROR (declarativo puro) o warning (con hooks, por si un `do` reubica en runtime). Un `path` (followPath) en cambio llega por distancia recorrida y SIEMPRE completa aunque no toque el waypoint, así que un destino inalcanzable en un path es solo warning visual (no cuelga); el path solo cuelga si tiene `loop: true` (nunca termina) y lo espera un arrive. Caza también un `arrive:<id>` huérfano que no espera a nada. (2) **ids duplicados** (`checkDuplicateIds`): dos objetos con el mismo id en una colección dejan al segundo indirigible (`byId` devuelve el primero) → ERROR; y avisa si un id de prop pisa uno de entidad. (3) **onDraw sin dibujar learners**: definir `hooks.onDraw` apaga el pase automático del elenco; si el hook no los dibuja (`draw.learner`/`drawSorted`/`_drawLearners`) y la escena declara entidades, warning (personajes invisibles). (4) **charts fuera de rango** (en `vChart`): datos o `target` fuera de `xDomain`/`yDomain`, o una curva que ocupa <15% del `yDomain` (aplastada), como warnings (la regla del checklist que el validador no atrapaba).
...
**Mirada dirigida (`lookAt`).** Por defecto las pupilas siguen el vector de movimiento; quietas, miran al frente. Con `entity.lookAt` (id de otra entidad o un punto `{x, y}`) un blob quieto orienta la mirada hacia ahí (el movimiento manda sobre `lookAt`). Vive en `draw.js`. Da mucha vida: úsalo en escenas con varios personajes. **A mitad de guion se cambia con el step declarativo `{ "lookAt": "<id>", "to": "<otroId>" | [x,y] | {x,y} }`** (sin `to`, o `to: null`, suelta la mirada): NO uses un `do` con `world.byId(...).lookAt = ...`.

**Vocabulario declarativo (menos `do`).** Steps que reemplazan patrones de `do` imperativo repetidos, para que el guion mande: `{ "lookAt": "<id>", "to": ... }` (arriba); `{ "jump": "<id>", "duration"? }` (salto expresivo, era `fx.jump`); `{ "set": { "<id>.prop": valor } }` escribe una propiedad de una entidad/prop por id (`{ "granada.seeds": 5 }`, `{ "puerta.open": true }`, `{ "gato.pose": "curl" }`) además de las claves de `world.state` sin punto; y el step `tween` acepta **selectores de grupo**: `{ "tween": "type:tree.alpha", "to": 0 }` anima esa propiedad en TODOS los props de ese `type`, y `{ "tween": "tag:primavera.alpha", "to": 0 }` en los props que declaren `tag` (columna del viraje de estación, antes un `do` con `filter`/`forEach`). Todo es reset-safe (los props se reconstruyen del config). En `scripts.js` (`execScriptStep`) y `validate.js` (STEP_KEYS, STEP_ENTITY_REFS, selectores de tween, `tag` en PROP_KEYS).
...
**Matemática estilo LaTeX sin dependencias.** Para ecuaciones formales (fracciones apiladas, raíces, griegas, operadores) hay un subconjunto de LaTeX delimitado con `$...$`, SIN traer KaTeX. `util.js: mathToHtml` parsea ese subconjunto a HTML (maneja llaves anidadas: `\frac{}{}`, `\sqrt{}`, `\Delta`...`\omega`, operadores `\ln \log \sin`..., relaciones `\le \ge \to \cdot \times \pm`...; las variables van en `<i>` itálica). `richToHtml` separa por `$` y manda los segmentos math a `mathToHtml` y el resto al sub/sup simple. Para el canvas hay DOS caminos según lo que quieras: `drawRichText`/captions usan `texToPlain`, que convierte `$...$` a texto plano legible (símbolos a Unicode, fracción **inline** `a/b`, dejando `_`/`^`) — cómodo para una caption, pero NO apila. Para notación **vertical apilada de verdad en el canvas** usa la primitiva `world.draw.math(x, y, tex, opts)` desde `onDraw` (motor de cajas en `util.js: drawMath`/`measureMath`; `draw.js: Draw.math`/`measureMath`). Cubre fracción apilada, raíz n-ésima (`\sqrt[n]{}`), sub/super combinables, símbolos, **operadores grandes con límites** (`\sum \prod \int \oint \bigcup`... apilados; integrales con límites laterales), **operadores tipo límite** (`\lim \max \min \sup \inf`), **delimitadores auto-escalables** (`\left( \right)`, `\left[ \left\{ \left|`...) y **matrices/casos** (`\begin{matrix|pmatrix|bmatrix|vmatrix|cases}...\end{}`), más espacios (`\, \; \quad`). Para auditar todos los constructos: `tools/math-test.html`. Ancla por `opts.align` (left|center|right) y `opts.valign` (baseline|middle|top|bottom); toma `px`, `color` (default el `fillStyle` actual), `weight`, `family`, `alpha`; devuelve `{ w, ascent, descent, height, x, baseline }` para posicionar partes contiguas (p.ej. un resultado resaltado en otro color: dibuja la derivación y el resultado como dos llamadas, midiendo con `measureMath`). Es la forma canónica para paneles de ecuaciones sobre el lienzo: NO vuelvas a dibujar fracciones a mano con `fillText` + una barra. Y para una escena DECLARATIVA (sin `onDraw`) hay un envoltorio, **`formulas` declarativas** (ver más abajo): NO necesitas `onDraw` solo para mostrar una ecuación. El estilo del HTML (variables itálicas, fracción con barra, dígitos lining, espaciado de operadores/relaciones) vive en el `<style>` del shadow root de `element.js` (`.math`, `.frac`, `.op`, `.rel`). Limitación: una ecuación LaTeX en un `label` HTML no se rasteriza bien en la grabación de video (el overlay se copia como texto plano); para video, las fórmulas formales conviene dibujarlas en el canvas con `world.draw.math` (entra en la grabación) o tenerlas en captions.
...
**Fórmulas declarativas.** `world.draw.math` sin escribir `onDraw`: un bloque `formulas` top-level y el step `formula` del guion. Cada fórmula: `{ id, tex, x, y, px?, color?, weight?, family?, align? (left|center|right), valign? (baseline|middle|top|bottom), alpha? (0 para revelarla luego), screen? (true = callout HUD fijo, inmune a cámara; default = espacio de mundo, un push-in la agranda), panel? }`. `tex` es un string O un **array de segmentos** `{ tex, color }` que se dibujan en fila (para resaltar un resultado en otro color, ej. el `1/2` ámbar del umbral de la escena 08). `panel: true` (o `{ title, bg, border, pad, radius, titleColor }`) dibuja la tarjeta de respaldo con encabezado en small caps. El step `{ "formula": "<id>", show?|hide?|alpha?, duration?, easing? }` la muestra/oculta/anima la opacidad con tween (espeja el step `chart`). Se dibujan (mundo tras los charts, pantalla bajo el HUD) y se re-ocultan en cada reset (vía `_initDeclarative`). Vive en `world.js: _drawFormulas`/`_initDeclarative`, `scripts.js` (step) y `validate.js` (`vFormula`). Es el camino declarativo para paneles de ecuaciones; la escena 08 (el umbral de inmunidad) lo usa. NO escribas `onDraw` solo para una fórmula.
