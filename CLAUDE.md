...
El catálogo (`index.html`) y la landing (`landing.html`) se eliminaron el 2026-06-03. El **2026-06-14 noesis fijó su forma definitiva: un motor de animaciones explicativas nativo de Claude.** No es un SaaS ni un producto self-serve para docentes no técnicos (esa hipótesis se probó y se descartó: ver abajo). Se usa a través de la skill `/noesis` (`~/.claude/skills/noesis/`): un autor con criterio, junto a Claude, dialoga el concepto, elige una mecánica, construye la escena con el vocabulario del motor, la valida con `tools/validate.mjs` y la previsualiza. La calidad la pone el autor, no un asistente foráneo improvisando. El repo es open source para quien lo maneje desde el código; quien no, no es el público. La visión completa está en `docs/noesis-vision.md`.

El aparato BYOA anterior ("trae tu propio asistente": `constructor.html`, `playground.html`, `manual-para-generar-escenas.md`, y los charters `docs/noesis-northstar.md` / `docs/noesis-mvp-scope.md`) se **retiró el 2026-06-14**. Su valor (las nueve mecánicas, las reglas de los tests, los ejemplos canónicos) ya vive en este documento, en `.claude/commands/noesis-scene.md` y en la skill, que es lo que Claude lee directamente. El motor, el validador y las formas (`engine/forms.js`) se conservan y caen de lleno en esta visión.
## Cómo previsualizar
...
## Rediseño pedagógico y reinicio del repertorio (2026-06-13 / 14)

Tras el lote BYOA y una prueba 100% humana (concepto: entropía) que salió VÁLIDA pero "un PowerPoint muerto" (cabezones parlantes con un gráfico), el usuario fijó el norte: **cada animación es una pieza de pedagogía, no un video**, y decidió reiniciar la capa pedagógica/de presentación SIN tocar el motor. El plan vive en `docs/noesis-rediseno-pedagogico.md`. Principio organizador: cada escena encarna UNA mecánica visual; el diálogo solo acompaña. Catálogo en 9 mecánicas: el viajero, la población, el umbral, la convergencia, el antes/después, la disección, la analogía, la cadena (dominó), la transformación en sitio (entropía/morph). Diferidas por solape: ensamblaje, propagación. **El repertorio se reinició el 2026-06-14.** Con el pivote a motor nativo de Claude (sin self-serve: ver `docs/noesis-vision.md`), se borraron las escenas 02-12 para reconstruir el repertorio deliberadamente bajo la modalidad nueva. Queda solo `01-netflix-prize`, la vara de calidad y el ejemplo de referencia de cómo usar el motor. El manual mecánica-primero se reescribió; los «siguientes pasos» originales de aquel plan (reconstruir el constructor, armar la galería como rampa única) quedaron OBSOLETOS con el mismo pivote. Lo que sigue vigente es el principio mecánica-primero. Se conservan motor, validador y las reglas de los 9 tests (ahora como checklist).

## Lecciones de los tests de campo (2026-06-12, modalidad BYOA retirada)

> La modalidad self-serve que estos tests probaban (docente + LLM foráneo) se retiró el 2026-06-14 (ver `docs/noesis-vision.md`). Los HALLAZGOS de ingeniería siguen vigentes: son el checklist de autoría que aplica igual cuando Claude construye una escena. Se conservan como bitácora de cómo se ganó cada regla.

Se simuló el flujo del docente: un agente con SOLO el manual (sin el motor ni la memoria del proyecto) generó una escena de interés compuesto a ciegas. La estructura salió sólida sin corrección (arco, tiempos, abstracción, props). El validador atrapó 1 error (`mood` inventado). Tres tropiezos que el validador NO atrapaba se convirtieron en mejoras: (1) signos de exclamación en textos editoriales (la sección 3 del manual los prohíbe pero `checkTextStyle` no los chequeaba: ahora sí, como warning); (2) `yFormat` usado como prefijo de moneda (`"$"` da "5000$": el manual ahora avisa que es sufijo); (3) `yDomain` del chart mal escalado, dejando la curva aplastada contra el piso y la `target` inalcanzable (el validador no puede atraparlo; el manual ahora enseña a ajustar el dominio a los datos). Lección de método: el manual maneja bien la estructura; los gaps están en el detalle semántico que el validador no ve, y cada uno se cierra en el manual o en `checkTextStyle`.

Segundo test BYOA (2026-06-12), didáctico: una escena del ciclo del agua (seguir una gota por sus etapas, fenómeno que se ve ocurrir). Confirmó que la regla de exclamación de hoy cambia el comportamiento del modelo (la gota habla serena, sorpresa en `mood`). Destapó dos vacíos del MANUAL que el test conversacional no podía mostrar, ambos de escenas espaciales/verticales: (1) crítico, el manual no documentaba `skybound`, así que la gota que se evapora hacia el cielo se clavaba en el horizonte (clamp de `learner.js`) y el mecanismo central fallaba en silencio; (2) el catálogo de pisos (`water`, `sand`, `snow`, `cobble`, `grid`) no estaba en el manual, así que el asistente inventó un prop `ocean` (no hay prop de mar: el agua es un piso). Ambos documentados. Pendiente de evaluar: un prop de cuerpo de agua (charco/laguna) además del piso `water`.

Tercer test BYOA (2026-06-12), familia «momento/atmósfera» (lo sublime romántico, el escenario carga el sentido). El asistente fue bien al kit atmosférico (cielo storm, música melancholic, lluvia, shake en el clímax), y la regla de exclamación se sostuvo por tercer test. Destapó un hueco real: **no había forma de cambiar el clima a mitad de escena**. El asistente inventó `{ "ambient": {...} }` como step (la profe pidió «que el clima cambie»); el `ambient` es solo config top-level. Como `tickAmbient` ya consulta `world._ambient.particles` vivo, se agregó el step declarativo **`weather`** (`scripts.js`): `{ "weather": "rain" }` arranca lluvia/nieve/petals/leaves/fireflies/stars a mitad de escena, `{ "weather": "none" }` la detiene (fireflies/stars pre-spawnean su población base). La oscuridad dinámica sigue por `tween ambient.darkness`. Manual aclarado (ambient es top-level, clima dinámico = step weather) y nudges (mountain es prop de suelo; letterbox para momentos solemnes).

Cuarto test BYOA (2026-06-12), familia «diálogo de ideas / mayéutica» (flotación y densidad, método socrático). Pasó el validador LIMPIO (cero errores): el arco emocional mayéutico salió impecable (determined→shocked→confused→cry→thinking→happy→reinforce), el «que escucha reacciona» en cada turno, y la regla de exclamación se sostuvo por 4º test. Meta-hallazgo: con el manual maduro el validador ya no atrapa errores de vocabulario, y los vacíos que quedan son de CALIDAD DIDÁCTICA, invisibles al validador. El concreto: **el asistente NARRÓ la demostración en vez de mostrarla** (la profe pidió un barco que flota y una moneda que se hunde; el asistente declaró el pond pero no puso `boat`/`coin` ni animó nada al agua, y los personajes reaccionan a algo invisible). Se reforzó el manual con una regla de oro dedicada en la sección 9 («muestra la demostración, no la narres») + chequeo en la sección 12, más nudges (hero true para protagonistas; references siempre presente en []). Sin cambios al motor: este hallazgo es 100% de manual.

Lote de 5 tests BYOA (2026-06-12), iterativo y autónomo (heliocentrismo, efecto invernadero, fotosíntesis, selección natural, oferta/demanda). Cosecha grande, varios hallazgos sistémicos (el mismo error en varios agentes a la vez):
- **`body` como string** (los 5): los LLM entregan `text.es.body` como UN string con `<p>...</p>` en vez de un array; `element.js` hacía `.map` sobre él y CRASHEABA el render del texto. Ahora `element.js` tolera string (lo envuelve, sin doble `<p>` si ya lo trae) y el validador lo avisa (warning, forma preferida = array).
- **`ambient` anidado en `canvas`** (3 de 5): el manual no listaba `ambient`/`charts`/`sets` como claves top-level. Ahora sí (sección 4), y el validador, ante una clave de `canvas` que es en realidad top-level, lo dice explícito («va al nivel superior, no dentro de canvas»).
- **`{ "focus": "off" }`** (2 de 5): confundieron la forma de apagar; es `{ "focus": "<id>", "off": true }`. Validador con mensaje dedicado; manual aclarado.
- **Foco sobre un chart** (oferta/demanda): era un push-in legítimo que el motor no soportaba; ahora `_focusPoint` resuelve un chart por id a su centro, y el validador lo acepta.
- **FX/movimiento sobre props** (appear "sol", reinforce "arbol", walk "pajaro"): las reacciones y `walk` son solo para personajes (`learner`); los props con vida (bird/fish/...) no se dirigen. Manual con sección dedicada (PERSONAJES vs PROPS) en §7, y narrador = `caption`, no un learner invisible.
- Menores (validador ya los caza o son de modelo): mood inventado («concerned»), `building` con `color1` en vez de `color`, `hero:false` en protagonistas, un fake `caption: "Ver nuevamente"`.
...
**Música ambiental opcional.** Cada escena puede declarar `meta.music` con uno de los moods soportados (`cosmic`, `melancholic`, `pastoral`, `epic`, `ancient`, `pedagogical`, `electronic`, `warm`). Cuando está presente, el engine muestra un botón ♪ a la derecha del lienzo (junto a fullscreen y reset) y el usuario decide si la activa. La música es procedural (Web Audio sin archivos externos): drone armónico generado en `engine/audio.js: createAmbientMusic` con osciladores + LFOs + delay feedback. Arranca silenciada por default. Para agregar moods nuevos, editar `MUSIC_PRESETS` en `audio.js`.

Para que la música tenga movimiento (no un drone plano), un preset puede declarar capas opt-in que `createAmbientMusic` interpreta: `chords` (array de acordes en Hz + `chordDur`: el drone se re-afina entre ellos), `arp` (patrón `{v,oct}` sobre el acorde actual + `arpStep`/`arpDur`/`arpVol`/`arpType` + `arpRest`, fracción 0..1 de silencios aleatorios; el arpegio además humaniza la dinámica por nota), `pulse` (latido grave suave + `pulseStep`/`pulseDur`/`pulseVol`) y `grow` (capa electrónica con `elecStep`/`elecVol` cuyo volumen sube a lo largo de `growDur` segundos). **Desde 2026-06-13 los OCHO moods son miniaturas con progresión de acordes y capas propias** (cosmic deriva sin pulso, epic marcha y crece, ancient es modal con tambor lejano, pedagogical es deliberadamente discreto...): un preset nuevo no puede ser un drone de `freqs` fijas, cansa en escenas largas. La página interna `tools/music-test.html` audiciona los ocho moods de un click; usarla tras cualquier cambio de audio.
...
**Aprendices siempre con pupilas.** Nunca uses `look: 'blank'`. Si el motivo es narrativo, pasale un vector de motion o un target en `onDraw`. El engine ya hace que la mirada siga la dirección de movimiento.
...
├── CHANGELOG.md           # bitácora del proyecto
